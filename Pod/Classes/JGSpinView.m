...
- (void)startOpenAnimating:(id)sender {
    if (displayTimer2) return;
    lastTimerDate = nil;
    displayTimer2 = [CADisplayLink displayLinkWithTarget:self selector:@selector(animationTimer:)];
    [displayTimer2 addToRunLoop:[NSRunLoop mainRunLoop] forMode:NSRunLoopCommonModes];
}

- (void)stopOpenAnimating:(id)sender {
    if (!displayTimer2) return;
    [displayTimer2 invalidate];
    displayTimer2 = nil;
}
